...
function GapRow({ text }: { text: string }) {
  return (
    <li className="flex items-start gap-2 text-[12.5px]" style={{ color: "#FCA5A5" }}>
      <span className="mt-0.5 text-[11px]" style={{ color: "#EF4444" }}>✕</span>
      <span className="leading-relaxed font-medium">{text}</span>
    </li>
  );
}
...
export function JobInsightSheet({
  ...
  // Read distilled_data directly — zero client-side LLM calls when available
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
          ...
          {/* ── Pending distillation banner (only when truly pending + no data) */}
          {isPending && (
            <div
            ...
            >
              ...
              <p className="text-xs leading-relaxed font-medium" style={{ color: "#A1A1AA" }}>
                The AI hasn&apos;t analysed this job yet. Hit Re-distill to extract the full role details now.
              </p>
              <button
          ...
          {/* ── Distilled Hooks (instant — from distilled_data) */}
          {hasDistilledData && distilled.hooks && distilled.hooks.length > 0 && (
            <section>
              <SectionLabel>AI Outreach Hooks</SectionLabel>
              <div
              ...
              >
                ...
                {distilled.hooks.map((hook, i) => (
                  <div
                    key={i}
                    className="text-[12px] leading-relaxed font-medium rounded-md p-2.5"
                    style={{
          ...
          {/* ── Skill Gaps (instant — from distilled_data) */}
          {hasDistilledData && distilled.gaps && distilled.gaps.length > 0 && (
            <section>
              <SectionLabel>Skill Gaps</SectionLabel>
              <ul className="space-y-1.5">
                {distilled.gaps.map((gap, i) => (
                  <GapRow key={i} text={gap} />
                ))}
              </ul>
            </section>
          )}
          ...
          {/* ── Tailored Bullets (instant — from distilled_data) */}
          {hasDistilledData && distilled.tailored_bullets && distilled.tailored_bullets.length > 0 && (
            <section>
              <SectionLabel>ATS-Optimised Bullets</SectionLabel>
              <ul className="space-y-1.5">
          ...
          {/* ── Fallback: About the Role (when no distilled data) */}
          ...
          {/* ── Distillation escape hatch (available even when distilled_data exists) */}
          <div className="pt-2">
            <button
              onClick={handleRedistill}
              disabled={isRedistilling}
              className="w-full flex items-center justify-center gap-2 rounded-lg py-2 text-[11px] font-bold transition-all hover:opacity-90 disabled:opacity-50"
              style={{
